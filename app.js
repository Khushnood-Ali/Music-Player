// Initial playlist from provided data with working audio URLs
let playlist = [
    {
        id: 1,
...
// Create demo audio track for testing
function createDemoAudio() {
    // Create a simple audio context for generating test tones
    try {
        ...
        tempAudio.addEventListener('loadedmetadata', () => {
            ...
            if (processed === total) {
                setTimeout(() => {
                    ...
                    // If no current track is playing, load the first uploaded track
...
function clearUploadedFiles() {
    ...
    if (confirm(`Are you sure you want to remove all ${uploadedTracks.length} uploaded files?`)) {
        ...
        // Filter out uploaded tracks
        playlist = playlist.filter(track => track.type !== 'uploaded');
        
        // Reset current track if it was an uploaded one
...
// Start progress updates with interval
...
function loadTrack(index) {
    if (index < 0 || index >= playlist.length) {
        ...
        albumCover.src = 'https://via.placeholder.com/300x300/333333/FFFFFF?text=🎵';
    ...
    // Update track counter and playlist highlight
    updateTrackCounter();
...
function updateProgress() {
    ...
    // Update duration display with actual duration if available
    const currentTrack = playlist[currentTrackIndex];
...
function deleteSelectedItems() {
    ...
    if (confirm(`Are you sure you want to delete ${selectedCount} selected track${selectedCount === 1 ? '' : 's'}?`)) {
        // Get selected tracks for cleanup
        ...
        if (playlist.length === 0) {
            currentTrackIndex = -1;
            loadTrack(-1); // This will show "no tracks available"
        } else {
...
function setupEventListeners() {
    ...
    // Handle dragging for progress and volume
    setupDragHandlers();
}
